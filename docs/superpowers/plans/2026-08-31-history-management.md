...
**에이전트는 이 파일을 직접 지우지 않는다.** `.claude/settings.json` 이 `Bash(rm *)` 와 `PowerShell(Remove-Item *)` 를 하드 deny 로 막아 두었다. 그 차단을 다른 수단(파이썬 `unlink`, `del` 등)으로 우회하는 것은 금지다 — 가드레일을 무력화하는 행위다.

대신 사용자에게 다음 한 줄을 요청하고 확인을 기다린다.

> 스키마가 바뀌었습니다. 프로젝트 루트의 `questions.db` 를 지워 주세요. 추적되지 않는 파일이라 커밋에는 영향이 없습니다.

앱은 다음 실행에서 새 스키마로 DB 를 다시 만든다. 파일이 남아 있으면 Task 7 의 스키마 게이트가 안내 문구를 띄운다.
