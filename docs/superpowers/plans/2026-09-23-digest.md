...
| `tests/conftest.py` | 새 레지스트리 캐시를 비운다 | 8 |
...
`conftest.py` 는 아직 정리본 레지스트리를 비우지 않는다. 이 태스크의
테스트는 레지스트리를 running 으로 만들지 않으므로 통과하지만,
Task 8 이 그 빈틈을 막는다.
...
- [ ] **Step 1: conftest 가 새 레지스트리를 비우게 한다**

`tests/conftest.py` 의 `app_db` 를 고친다. 이걸 먼저 하지 않으면
아래 테스트들이 서로의 레지스트리 상태를 물려받는다.
