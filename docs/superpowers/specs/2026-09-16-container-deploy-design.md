...
services:
  app:
    build: .
    image: ghcr.io/susoterran/notebooklm-st:${NOTEBOOKLM_ST_TAG:-latest}
...
**이미지.** 홈서버는 8절의 워크플로가 GHCR 에 올린 이미지를
`docker compose pull` 로 받아 띄운다. 태그는 `NOTEBOOKLM_ST_TAG`
(기본 `latest`)로 정하고, 배포 디렉터리의 `.env` 에 릴리스 태그를 적어
고정할 수 있다. 원격 로그인 사이드카(`login-browser`)도 같은 변수를
쓴다(`docs/superpowers/specs/2026-09-26-remote-login-design.md` 9.2).
`build: .` 은 소스에서 굽고 싶을 때(`docker compose up -d --build`)를
위해 남긴다.
...
원격 로그인 사이드카 이미지(`ghcr.io/susoterran/notebooklm-st-login-browser`)
도 같은 잡에서 같은 게이트를 지나 같은 태그로 올라간다
(`docs/superpowers/specs/2026-09-26-remote-login-design.md` 9.3).

홈서버는 이 이미지를 받아 쓴다. 굽지 않는다. 이유는 둘이다.
사이드카는 크로미움을 담아 홈서버에서 구우면 몇 분씩 걸리고 네트워크·
apt 상태에 흔들린다. 그리고 게이트가 검사한 아티팩트가 그대로 돌아야
게이트가 의미를 가진다 — 홈서버에서 다시 구우면 그 사이에 바뀐 베이스
이미지와 패키지가 검사 없이 들어간다.
...
스캔 빌드가 만든 **바로 그 이미지**에 compose 가 찾는 이름을 붙여
돌린다. 잡 env 로 `NOTEBOOKLM_ST_TAG=ci` 를 두고 `ghcr.io/…:ci` 를
붙이므로, 이름이 로컬에 이미 있어 compose 는 다시 굽지도 GHCR 에서
받지도 않는다. 검사 대상과 게시 대상이 같은 아티팩트가 된다. `ci`
태그는 롤링 태그와 겹치지 않고 게시되지 않는다. 홈서버와 같은 설정(비root·`read_only`·
...
왕복, 재시작 후 지속성)은 실제 홈서버와 구글 계정이 필요하다. 이것은
사람이 도는 체크리스트로 남는다. 러너(amd64)와 홈서버(x86_64)는
아키텍처가 같으므로 게시된 이미지가 그대로 돈다.
...
- `docker compose pull && docker compose up -d`, 태그 고정과 업데이트,
  GHCR 패키지 공개 범위(비공개면 `docker login ghcr.io`)
...
| # | 확인 | 통과 기준 | 누가 |
|---|---|---|---|
| 1 | 스캔 빌드 | 성공. `--frozen` 이 잠금 파일 불일치를 여기서 잡는다 | 워크플로 |
...
그룹 설치도 CI 러너에서 문제없이 끝난다. 사람 몫은 홈서버에서 컨테이너를
띄워 자격증명 반입까지 확인했다. 설계 시점에 모르던 홈서버의 아키텍처
(x86_64)·도커 버전·사용자 UID 는 이 실행으로 해소됐다.
...
- **멀티아치 이미지** — 홈서버가 x86_64 라 러너(amd64)가 굽는 단일
  아키텍처 이미지로 충분하다. 다른 아키텍처의 배포 대상이 생길 때
  다시 본다.
