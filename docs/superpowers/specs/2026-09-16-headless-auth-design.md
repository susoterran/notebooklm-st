...
| Slack 알림 | **범위 밖** | 앱에는 무인으로 도는 것이 없다. 사용자가 버튼을 눌러야 NotebookLM 을 건드리고, 그러면 만료는 그 자리에서 화면에 보인다 |
| 인증 상태 값 객체 | **도입하지 않음** | 지금 필요 없는 구조다. 필요해지면 그때 넣는다 |
...
판정과 문구가 화면 밖에 있으므로, 화면이 아닌 곳에서 인증을 확인할 일이
생겨도 `services/auth.py` 를 부르고 `core/errors.py` 의 같은 문구를 쓰면
된다. 화면과 다른 말을 할 수 없다.
