...
5. **정리본** 화면에서 저장된 요약본 여럿을 골라 하나의 글로 정리합니다. 정리 지시는 질문 관리에 등록된 질문 중 하나를 골라 씁니다. 제목은 NotebookLM 이 정리와 함께 지은 주제로 `[정리] <주제>` 가 기본값이며 고쳐 쓸 수 있습니다. 저장하면 Outline 에 문서가 생기며, 정리본은 로컬에 남지 않습니다.
