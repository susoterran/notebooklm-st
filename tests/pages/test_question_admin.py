...
def test_adding_a_question_saves_title_and_text(app_db) -> None:
    ...
    assert not app.exception
    saved = questions.list_questions(app_db)
...
def test_blank_title_is_rejected(app_db) -> None:
    ...
    assert questions.list_questions(app_db) == []
...
def test_duplicate_title_is_rejected(app_db) -> None:
    ...
    questions.add_question(app_db, "같은 제목", "첫째 본문")
    app = v1.AppTest.from_function(script)
    app.run()
    ...
    assert not app.exception
    assert len(app.error) == 1
    texts = [question.text for question in questions.list_questions(app_db)]
    assert texts == ["첫째 본문"]
...
def test_updating_a_question_saves_title_and_text_without_swap(
    ...
    questions.add_question(app_db, "옛 제목", "옛 본문")
    app = v1.AppTest.from_function(script)
    app.run()
